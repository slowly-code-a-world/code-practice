/*
10.7 Imagine a web server for a simplified search engine. This system has 100
    machines to respond to search queries, which may then call out using
   processSearch(string query) to another cluster of machines to actu-
  ally get the result. The machine which responds to a given query is chosen at
...
// use hash table as cache, when data change invalidate the cache
